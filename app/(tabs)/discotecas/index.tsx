...
export default function DiscotecasScreen() {
  const items = LUGARES.filter(l => l.tipo === 'Discoteca');
  return (
    <View style={styles.container}>
      <Text style={styles.maintitle}>Discotecas</Text>
    <FlatList
      contentContainerStyle={{ padding: 12 }}
      data={items}
      keyExtractor={(i) => i.id}
      renderItem={({ item }) => (
  <Link href={`/lugares/${item.id}?from=discotecas`} asChild>
...
const styles = StyleSheet.create({
  ...
  maintitle: { fontSize:22, fontWeight:'700', color:'#1e293b', marginVertical:1, textAlign:'center',marginTop:35 },
});
